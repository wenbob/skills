...
description: >
  ...
  仅当用户明确要求同步、清理、核对或更新项目文档/Agent 记忆时使用，例如
  "整理文档"、"同步项目文档"、"更新记忆"、"清理过期文档"、"准备交接"、
  "/sync" 或 "/neat"。普通的"整理一下"、"收尾"或阶段完成不应自动触发写入。
...
> **Agent 记忆系统的位置和写入规则因平台而异。** 先读取当前平台提供的记忆说明和权限规则，再定位实际目录；不要把 `AGENTS.md` 当作 Codex 记忆系统的唯一实现。完整路径参考见 [references/agent-paths.md](references/agent-paths.md)，但平台当前指令优先于该参考。若平台没有可写记忆机制，跳过记忆层。

## 授权边界

- 用户只要求“检查、审计、看看是否过期”时，默认只读并输出建议，不修改文件。
- 只有用户明确要求同步或更新项目文档时，才可修改 `README.md`、`AGENTS.md`、`CLAUDE.md` 或 `docs/`，且范围限于用户指定或本次任务直接涉及的项目。
- 只有用户明确要求更新记忆时，才可写入 Agent 记忆；必须遵循当前平台规定的位置和更新机制。
- 删除文件、删除记忆或跨项目同步必须有明确范围。无法确认内容是否废弃时，保留并列入“未处理”，不得自行扩大清理范围。
...
如果用户明确要求同步或更新，必须实际修改获准范围内的文件，不能只描述建议。如果用户只要求检查或审计，则停留在只读报告，不得写入。创建或删除文件必须确有必要并符合上面的授权边界。
...
**发现之前的同步漏了东西**：如果属于当前授权范围就修复；超出范围则列入“未处理”，说明需要何种授权，不要擅自扩展到其他项目或记忆。
