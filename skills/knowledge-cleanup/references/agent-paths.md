...
| 用途 | 路径 |
|---|---|
| 跨会话记忆 | 由当前 Codex 运行环境提供；先读取本次会话中的 memory 说明，不假定固定路径或可直接写入 |
...
`AGENTS.md` 是指令文件，不等同于记忆数据库。某些 Codex 运行环境会提供独立的记忆目录、索引或受控更新机制；只有用户明确要求更新记忆时，才按当前会话给出的规则操作。不要把项目事实默认写进全局 `AGENTS.md`，也不要自行猜测 fallback 文件名。
