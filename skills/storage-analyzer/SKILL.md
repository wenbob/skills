...
description: >
  ...
  可执行处置方案，生成排版精美、可折叠的 HTML 报告。扫描和默认报告全程只读；仅当
  用户在查看结果后明确要求执行清理时，才可启用带操作按钮的本地服务。务必在以下场景
...
对 macOS 或 Windows 做一次只读存储分析，产出 HTML 报告。流程：扫描 → 分析分级 → 生成网页 → 打开。

## 铁律

- **分析阶段和默认报告只读。** 扫描、统计、分类和生成静态报告时，禁止删除、移动、清空回收站、修改权限或改写被扫描目录。
- **清理必须二次授权。** 先交付分析结果和候选路径；只有用户在看到具体项目、路径、预计释放空间和风险后明确要求清理，才可启用服务模式。授权范围外的路径一律不可操作。
- **优先可逆操作。** 获得授权后默认移到回收站；只有用户再次明确选择不可逆删除时才允许直接删除。红灯项始终不提供删除操作。
...
仅在用户已经明确授权启用清理服务时，🟢 项才填写 `trash_paths`（具体获准处理的绝对路径数组，区别于人类可读的 `path` 展示字段）。静态只读报告不得依赖删除能力。
...
仅在用户审阅静态报告后，明确授权对列出的具体项目进行清理时，才启用服务模式：
```bash
python3 scripts/server.py /tmp/storage_analysis.json   # 自动开浏览器，Ctrl+C 停
```
`server.py` 起在 127.0.0.1 + 随机端口 + 随机 token。🟢 项默认只使用「移到废纸篓」；只有用户明确要求不可逆删除时才显示或使用「直接删除」。🟡 项只允许打开位置，或在授权覆盖安全子路径时移到废纸篓；红灯项不给删除按钮。安全模型使用三套白名单：`rm` 只允许用户明确批准直接删除的绿灯路径；`trash` 允许获准的绿灯/橙灯 `trash_paths`；`open` 允许上述路径及橙灯真实 `path`。所有请求必须通过 realpath、HOME 范围、token 和 Host 校验，并在浏览器内再次确认。
